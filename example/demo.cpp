    ...
    ui::hbox(widgets_panel)
        << ( ui::vbox().layout().justify().stretch()
            ...
            << ui::button(widgets_panel, "button")
                ...
                .layout().justify()
            << ( ui::hbox().layout().justify()
                << ui::checkbox(widgets_panel, "checkbox")
                    .indeterminate()
                    .on_toggle(utility::make_handler("checkbox", "toggle"))
                    .layout().stretch()
                << ui::tristate_checkbox(widgets_panel, "tristate_checkbox")
                    .indeterminate()
                    .on_toggle(utility::make_handler("tristate_checkbox", "toggle"))
                    .layout().stretch()
               )
            << ui::hline(widgets_panel).layout().justify()
            << ui::stringbox(widgets_panel, "stringbox")
                .on_edit(utility::make_handler("string", "edit"))
                .layout().justify()
            << ui::passwordbox(widgets_panel, "123456")
                .on_edit(utility::make_handler("passwordbox", "edit"))
                .layout().justify()
            << ui::textbox(widgets_panel, "textbox")
        ...
        << ( ui::vbox().layout().justify().stretch()
            << ui::listbox(widgets_panel,
                           boost::assign::list_of("listbox 1")("listbox 2")("listbox 3"))
                .on_select_event(utility::make_event_handler<ui::index_event>("listbox", "select"))
                .on_activate_event(utility::make_event_handler<ui::index_event>("listbox", "activate"))
